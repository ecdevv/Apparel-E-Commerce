...
import { validateURL, validateProduct, getSelectedOption } from '@/server/mockValidations';
import './product.css'
...
const ProductDetails = ( {searchParams}: {searchParams: {name: string, id: string, option: string, size: string}} ) => {
  ...
  // Call for the update url function run the UpdateURL component to update the url first
  const validateURLResponse = validateURL(product, selectedOption, selectedSize);
  const newURL = validateURLResponse.url;

  return (
    <>
      <UpdateURL searchParams={searchParams} url={newURL} />
