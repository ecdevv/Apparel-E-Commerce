...
const ProductsDetails = ({searchParams, Products}: {searchParams: {category: string, tags: string[]}, Products: Product[]}) => {
  ...
  const displayedProducts = (searchParams.category === 'all' || !searchParams.category)
    ...
    : Products.filter(product => {
        const hasCategory = product.tags.includes(searchParams.category);
        const hasTags = Array.isArray(searchParams.tags) && searchParams.tags.length > 0 ? searchParams.tags.every(tag => product.tags.includes(tag)) : false;
        
        if (searchParams.category === 'new') {
          const now = new Date('07-02-2024');
          const sixMonthsAgo = new Date(now.getFullYear(), now.getMonth() - 6, now.getDate());
          
          return product.options.some(option => option.releaseDate && new Date(option.releaseDate) >= sixMonthsAgo);
        }

        // if (searchParams.category === 'popular') {
        //   const topProducts = Products.sort((a, b) => {
        //     const aSales = a.options.reduce((acc, option) => acc + option.sizes.reduce((acc2, size) => acc2 + (size.sales || 0), 0), 0);
        //     const bSales = b.options.reduce((acc, option) => acc + option.sizes.reduce((acc2, size) => acc2 + (size.sales || 0), 0), 0);
        //     const aStock = a.options.reduce((acc, option) => acc + option.sizes.reduce((acc2, size) => acc2 + (size.sales || 0) + size.stock, 0), 0);
        //     const bStock = b.options.reduce((acc, option) => acc + option.sizes.reduce((acc2, size) => acc2 + (size.sales || 0) + size.stock, 0), 0);
        //     const aSoldPercentage = aSales / aStock;
        //     const bSoldPercentage = bSales / bStock;
        //     return bSoldPercentage - aSoldPercentage;
        //   }).slice(0, 5);
        ...
        if (searchParams.category === 'sales') {
          ...
          return hasCategory || hasTags || hasDiscount;
        }
  ...
  useEffect(() => {
    setSortCriteria (
      searchParams.category === "new" 
      ? SortCriteria.NEW_ARRIVALS 
      : searchParams.category === "popular" 
        ? SortCriteria.BEST_SELLERS
    ...
    switch (criteria) {
      case SortCriteria.PRICE_ASC:
        return products.sort((a, b) => 
          (a.options[selectedOptions[a.product_id] || 0].price * (1 - a.options[selectedOptions[a.product_id]].discount)) 
          - (b.options[selectedOptions[b.product_id] || 0].price * (1 - b.options[selectedOptions[b.product_id]].discount))
        );
